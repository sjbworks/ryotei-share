---
allowed-tools: Bash(gh pr diff:*), Bash(gh pr comment:*), Bash(gh pr review:*), Bash(gh pr checks:*), Bash(echo *), Bash(head *), Write, Read
...
3. diff だけでは判断できない箇所がある場合のみ `Read` ツールで該当ファイルを読む。その際は必ず `limit: 150` を指定する。自動生成ファイル（`__generated__` を含むパス）は読まない
