---
allowed-tools: Bash(gh pr view:*), Bash(gh pr diff:*), Bash(gh pr comment:*), Bash(gh pr review:*), Bash(gh pr checks:*), Bash(echo $GITHUB_REF:*)
...
1. 環境変数から PR 番号を取得する: `echo $GITHUB_REF | grep -oE '[0-9]+'`（例: `refs/pull/51/merge` → `51`）
2. `gh pr diff <PR番号> --patch | head -n 500` で変更差分の先頭を確認する。差分が大きい場合は `gh pr view <PR番号> --json files` で変更ファイル一覧を取得し、重要なファイルのみ `Read` で読み込む
3. 変更されたファイルを必要に応じて `Read` で読み込む（大きいファイルは offset/limit を使う）
