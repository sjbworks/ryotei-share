...
- PR 番号は必ず環境変数 `$PR_NUMBER` から取得する: `echo $PR_NUMBER`

## 手順

1. `echo $PR_NUMBER` で PR 番号を確認する
...
6. `gh pr comment $PR_NUMBER --body-file /tmp/review-comment.md` でコメントを投稿する
...
コメント投稿後、以下の **両方** を満たす場合のみ `gh pr review $PR_NUMBER --approve` を実行する。
