...
2. `gh pr diff $ARGUMENTS | head -500` で変更差分を確認する
3. 以下のチェックリストに従ってレビューする
4. レビュー結果を `Write` ツールで `/tmp/review-comment.md` に保存する
5. `gh pr comment $ARGUMENTS --body-file /tmp/review-comment.md` でコメントを投稿する
