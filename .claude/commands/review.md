---
allowed-tools: Bash(gh pr view:*), Bash(gh pr diff:*), Bash(gh pr comment:*), Bash(gh pr review:*), Bash(gh pr checks:*), Bash(echo *), Bash(head *)
...
1. `echo $PR_NUMBER` で PR 番号を取得する
2. `gh pr diff $PR_NUMBER | head -300` で変更差分を確認する
3. `gh pr view $PR_NUMBER --json files` で変更ファイル一覧を確認し、重要なファイルのみ `Read` で読み込む（大きいファイルは offset/limit を使う）
4. 以下のチェックリストに従ってレビューする
5. 「出力フォーマット」に従ってレビュー結果を作成し、`gh pr comment $PR_NUMBER --body "レビュー結果"` で PR にコメントを投稿する
...
レビューコメントを投稿した後、以下の **両方** を満たす場合のみ `gh pr review $PR_NUMBER --approve -b "CI が通過しており、Critical な問題がないため Approve します。"` を実行する。

**条件 1: Critical がない**
- 上記レビューで 🔴 Critical が「なし」であること

**条件 2: CI がすべて通過している**
- `gh pr checks $PR_NUMBER` を実行し、全チェックが `pass` であること
