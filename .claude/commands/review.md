...
3. 一覧から `package-lock.json` / `package.json` / `*.lock` などビルド成果物・依存関係ファイルを除外し、レビュー対象ファイルを絞り込む
4. 各レビュー対象ファイルの diff を `gh pr diff $ARGUMENTS -- <ファイルパス> | head -200` で取得する
5. 以下のチェックリストに従ってレビューする
6. レビュー結果を `Write` ツールで `/tmp/review-comment.md` に保存する
7. `gh pr comment $ARGUMENTS --body-file /tmp/review-comment.md` でコメントを投稿する
8. Approve 条件を満たしているか判断し、該当すれば Approve する
...
このレビューは CI（lint/typecheck/test/build）がすべて通過した後に実行される。
コメント投稿後、🔴 Critical が「なし」であれば `gh pr review $ARGUMENTS --approve` を実行する。
Critical がある場合は Approve せず、レビューコメントのみを残す。
